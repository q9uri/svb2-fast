# Usage: .venv/bin/python -m scripts.sig.decode

from pathlib import Path

import librosa

from style_bert_vits2.sig import TriSigAudio, decode_sig_morse
...
audio_morse_files = [
    file for file in output_morse_dir.rglob("*") if is_audio_file(file)
]
...
def decode_text():
    ...
    for audio_file in audio_files:
        ...
        (
            decode_text,
            decode_text_wavmark,
            decode_text_audioseal,
        ) = trisig.decode_sig(audio, sr)
        decode_texts.append(
            f"{audio_file},decoded_codease|{decode_text},decoded_wavmark|{decode_text_wavmark},decoded_audioseal|{decode_text_audioseal}"
        )
