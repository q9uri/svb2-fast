...
def sig_audio(base_audio: np.ndarray, key: str, fs=44100) -> tuple[np.ndarray, int]:
    freqs_list = [
        {
            "C4": 261.63,
            "D4": 293.66,
    ...
    finary_out = base_audio.copy()
    for freqs in freqs_list:
        interval = freqs["interval"]
        ...
        output_audio = np.zeros_like(base_audio)
        ...
        finary_out += output_audio

    return finary_out, fs
